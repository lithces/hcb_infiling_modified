...
data = sep_sents(' '.join(brown.words()), 0, tokenizer=tokenizer, max_toks = context_length)
...
for batch_num in range(num_experiments):
  ...
  masked_inputs_batch, true_ids_batch = mask_tokens_batch(input_ids, masked_positions, mask_token_id=tokenizer.mask_token_id)
